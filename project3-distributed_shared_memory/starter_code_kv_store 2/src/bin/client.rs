...
fn get_shard_info(
    ...
    if let Err(e) = send(ctx, &controller_pid, &Operation::GetShardInfo(shard_id)) {
        println!("Client failed to query shard info: {:?}", e);
        return Err(e);
    } else {
        loop {
            match recv(ctx, 1000) {
                Ok(responses) => {
                    ...
                    let (pid, operation) = &responses[0];
                    if *pid != *controller_pid {
                        return Err(Error::new(ErrorKind::InvalidInput, "Invalid response"));
                    }
                    let result: Result<ShardInfo, Error> = match operation {
                        Operation::GetShardInfoRes(shard_info) => Ok(shard_info.clone()),
                        _ => Err(Error::new(ErrorKind::InvalidInput, "Invalid response")),
                    };
                    return result;
                }
                Err(e) => {
...
fn run_client(
    ...
    while operation_index < num_operations {
        ...
        let server_pid = server_pids[(self_pid % (server_pids.len() as Pid)) as usize]; // always send to one server
        let begin_time = get_timestamp().unwrap();
        if let Err(e) = send(&mut ctx, &server_pid, operation) {
